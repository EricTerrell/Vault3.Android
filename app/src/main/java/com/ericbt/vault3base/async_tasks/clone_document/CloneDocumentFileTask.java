...
package com.ericbt.vault3base.async_tasks.clone_document;
...
public class CloneDocumentFileTask extends AsyncTask<CloneDocumentFileTaskParameters, Void,
		CloneDocumentFileTaskResult> {
	private CloneDocumentFileTaskParameters parameters;
	
	@Override
	protected CloneDocumentFileTaskResult doInBackground(
			CloneDocumentFileTaskParameters... params) {
		parameters = params[0];
		...
		try {
			...
			Log.e(StringLiterals.LogTag,
					String.format("CloneDocumentTask: Exception %s", ex.getMessage()));
			ex.printStackTrace();
		...
		if (result.getException() != null) {
			new AlertDialog.Builder(parameters.getFileActivity())
					...
					.setMessage(String.format(
							"Cannot copy %s", parameters.getSourceDocumentUri().toString()))
