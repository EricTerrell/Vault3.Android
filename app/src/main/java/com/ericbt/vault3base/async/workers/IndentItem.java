...
public class IndentItem extends Async {
    public void indentItem(OutlineItem outlineItem, Vault3 vault3Activity) {
        submit(() -> {
            Throwable exception = null;
            int newParentID = -1;

            try {
                newParentID = Globals.getApplication().getVaultDocument().indent(outlineItem);
            } catch (Throwable ex) {
                Log.e(StringLiterals.LogTag,
                        String.format("IndentItem: Exception %s", ex.getMessage()), ex);
