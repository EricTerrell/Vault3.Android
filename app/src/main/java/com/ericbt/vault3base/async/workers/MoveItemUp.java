...
public class MoveItemUp extends Async {
    public void moveItemUp(OutlineItem outlineItem, Vault3 vault3Activity) {
        submit(() -> {
            Throwable exception = null;

            try {
                Globals.getApplication().getVaultDocument().moveUp(outlineItem);
            } catch (Throwable ex) {
                Log.e(StringLiterals.LogTag, String.format("MoveItemUp: Exception %s",
                        ex.getMessage()), ex);
